...
// 최종 보고서용: 실행 시작 시 설정, 완료 시 사용
let runState = { startTime: null, runConfig: null, initialViewerCount: null, lastViewerCount: null, estimatedMin: null };
...
function run() {
    ...
    instances = Math.min(Math.max(instances, 1), usePuppeteer ? 2000 : 1000);

    let maxConcurrent = parseInt(document.getElementById('maxConcurrent').value) || 12;
    maxConcurrent = Math.min(Math.max(maxConcurrent, 1), 100);
    ...
    if (usePuppeteer) {
        runPuppeteerMode(url, instances, minDelay, maxDelay, maxConcurrent);
    } else {
        runFetchMode(url, instances, minDelay, maxDelay);
    }
}

// Puppeteer 모드: 실제 브라우저·플레이어 로드 → 시청자 수 반영 가능 (한번에 N명씩 동시 시청)
function runPuppeteerMode(url, instances, minDelaySec, maxDelaySec, maxConcurrent) {
    ...
    fetch('/api/start', {
        ...
        .then(function (result) {
            ...
            startTime = new Date();
            runState.startTime = startTime;
            runState.runConfig = { url: url, instances: instances, maxConcurrent: concurrent, minDelay: minDelaySec, maxDelay: maxDelaySec };
            runState.initialViewerCount = null;
            runState.lastViewerCount = null;
            ...
            if (videoId) {
                ...
                fetchYouTubeViewerCount(videoId).then(function (result) {
                    ...
                    lastViewerCount = n;
                    runState.initialViewerCount = n;
                    runState.lastViewerCount = n;
                        ...
                        viewerPollInterval = setInterval(function () {
                            ...
                            fetchYouTubeViewerCount(videoId).then(function (res) {
                                if (res.viewerCount != null) {
                                    ...
                                    runState.lastViewerCount = res.viewerCount;
...
function finishPuppeteer() {
    ...
    showFinalReport();
    startTime = null;
    addLog('success', '완료 (실제 브라우저 모드) · 최종 보고서를 확인하세요.');
}
...
fetch('/api/capabilities')
    ...
    .then(function (data) {
        ...
        if (usePuppeteer && typeof updateEstimatedTime === 'function') updateEstimatedTime();
    })
    .catch(function () {
        usePuppeteer = false;
        if (modeNoticeEl) {
...
// 예상 소요 시간 계산 (요청 수 / 동시 실행 수 * 세션당 약 2.5분, Puppeteer 모드만)
function updateEstimatedTime() {
    var panel = document.getElementById('estimatedTimeHint');
    if (!usePuppeteer) { if (panel) panel.style.display = 'none'; return; }
    var instances = parseInt(document.getElementById('instances').value) || 20;
    var maxConcurrent = parseInt(document.getElementById('maxConcurrent').value) || 12;
    if (instances <= 0 || maxConcurrent <= 0) return;
    var estimatedMin = Math.max(1, Math.ceil((instances / maxConcurrent) * 2.5));
    var el = document.getElementById('estimatedTimeMin');
    if (el) el.textContent = estimatedMin;
    if (panel) panel.style.display = 'block';
}

// 최종 보고서 표시
function showFinalReport() {
    var panel = document.getElementById('finalReportPanel');
    var content = document.getElementById('finalReportContent');
    if (!panel || !content) return;
    var start = runState.startTime;
    var end = new Date();
    var durationSec = start ? Math.floor((end - start) / 1000) : 0;
    var target = parseInt(totalVisitsEl ? totalVisitsEl.textContent : 0) || 0;
    var completed = parseInt(completedSessionsEl ? completedSessionsEl.textContent : 0) || 0;
    var failed = parseInt(failedSessionsEl ? failedSessionsEl.textContent : 0) || 0;
    var successRate = target > 0 ? Math.round((completed / target) * 100) : 0;
    var cfg = runState.runConfig || {};
    var vStart = runState.initialViewerCount;
    var vEnd = runState.lastViewerCount;
    var vChange = (vStart != null && vEnd != null) ? (vEnd - vStart) : null;
    var startStr = start ? start.toLocaleString('ko-KR') : '-';
    var endStr = end.toLocaleString('ko-KR');
    var durationStr = formatTime(durationSec);
    var estStr = runState.estimatedMin != null ? '약 ' + runState.estimatedMin + '분' : '-';
    var html = '<table><tbody>';
    html += '<tr><th>실행 시작</th><td>' + startStr + '</td></tr>';
    html += '<tr><th>실행 종료</th><td>' + endStr + '</td></tr>';
    html += '<tr><th>실제 소요 시간</th><td class="report-highlight">' + durationStr + '</td></tr>';
    html += '<tr><th>예상 소요 시간(참고)</th><td>' + estStr + '</td></tr>';
    html += '<tr><th>URL</th><td>' + (cfg.url || '-') + '</td></tr>';
    html += '<tr><th>요청 수(목표)</th><td>' + (cfg.instances != null ? cfg.instances : target) + '</td></tr>';
    html += '<tr><th>동시 실행 수</th><td>' + (cfg.maxConcurrent != null ? cfg.maxConcurrent : '-') + '</td></tr>';
    html += '<tr><th>최소/최대 대기(초)</th><td>' + (cfg.minDelay != null ? cfg.minDelay : '-') + ' / ' + (cfg.maxDelay != null ? cfg.maxDelay : '-') + '</td></tr>';
    html += '<tr><th>완료</th><td class="report-highlight">' + completed + '</td></tr>';
    html += '<tr><th>실패</th><td>' + failed + '</td></tr>';
    html += '<tr><th>성공률</th><td>' + successRate + '%</td></tr>';
    if (vStart != null || vEnd != null) {
        html += '<tr><th>YouTube 시작 시청자</th><td>' + (vStart != null ? vStart.toLocaleString() + '명' : '-') + '</td></tr>';
        html += '<tr><th>YouTube 종료 시청자</th><td>' + (vEnd != null ? vEnd.toLocaleString() + '명' : '-') + '</td></tr>';
        html += '<tr><th>YouTube 변화</th><td class="report-highlight">' + (vChange != null ? (vChange >= 0 ? '+' : '') + vChange.toLocaleString() + '명' : '-') + '</td></tr>';
    }
    html += '</tbody></table>';
...
// Puppeteer 모드일 때만 예상 소요 시간 표시
if (document.getElementById('instances') && document.getElementById('maxConcurrent')) {
    function attachEstimatedTimeListeners() {
        document.getElementById('instances').addEventListener('input', updateEstimatedTime);
        document.getElementById('maxConcurrent').addEventListener('input', updateEstimatedTime);
    }
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', attachEstimatedTimeListeners);
    } else {
        attachEstimatedTimeListeners();
    }
}
